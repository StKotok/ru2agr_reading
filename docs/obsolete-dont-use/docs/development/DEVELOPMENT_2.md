...
- [x] Распарсить `docs/clear-bible-alignments/SBLGNT-RUSSYN-manual.json` (89 248 записей)
- [x] Для каждой записи: source ID (греч. токен) → позиция в grcTokens, target ID (рус. токен) → позиция в русских словах
- [x] Построить alignment `{ru, gr}` из manual ID-соответствий
- [x] Для стихов без manual: Strong + sequential consumption (Map<strong, usageCount>)
- [x] Для слов без Strong: regex fallback из core.json
- [x] Перегенерировать alignment: `npm run build:data`
- [x] Вывести статистику: покрытие manual / Strong / regex / без alignment
- [x] Верификация: Ин 1:1 — три «Слово» → три разных λόγος по порядку
- [x] Верификация: Мк 1:1 — все 6 значимых слов выровнены корректно
...
- [x] `stripDiacritics`: λόγος → λογος, ἄνθρωπος → ανθρωπος, финальная σ/ς сохраняется
- [x] `finalSigma`: σ → ς перед пробелом/пунктуацией/концом
- [x] `preserveCase`: русский регистр → греческий регистр (Слово → Σλοβο, СЛОВО → ΣΛΟΒΟ)
- [x] `getRules`: возвращает 38 правил, диграфы раньше одиночных
...
- [x] Использовать реальные данные из `data/bibles/syn/mark.json` и `data/bibles/grc/mark.json` (Мк 1:1)
- [x] Форма ≠ лемма: «Евангелия» → εὐαγγελίου (род. падеж), а не εὐαγγέλιον (лемма)
- [x] `forms: 'lemma'` → лемма, `forms: 'all'` → реальная форма
- [x] Слово не из словаря → plain
- [x] Невыровненный стих → fallback (возвращает plain или вызывает word-layer)
- [x] Пунктуация: «Христа,» → заменяется на «Χριστοῦ» + запятая отдельным plain-сегментом
...
- [x] При замене слова с пунктуацией (напр. «Христа,»): отделить знаки препинания, выдать сегмент `{ greek, original: wordWithoutPunct }` + `{ plain: punct }` после
- [x] Полный regex пунктуации: `/[.,;:!?;—\-–"«»„"()\[\]'¿¡]+$/g` (включая греческий вопросительный знак `;` U+037E, кавычки, скобки)
- [x] Разделитель между словами `{ plain: ' ' }` — последнее слово стиха не должно иметь пробела после
- [x] Обновить тесты (из задачи 0.2)
...
- [x] В `mount()`: параллельно с syn книгой загружать grc книгу (`loadBook('grc', bookId)`)
- [x] В `renderWindowed()`: для каждого стиха находить grcVerse (grcBook.chapters[chN-1].verses[vN-1]) и alignment (verse.alignment)
- [x] Передавать `grcTokens` и `alignment` в `composeCtx`
- [x] При ошибке загрузки grc (нет сети) — режим 4 деградирует в режим 3 с тостом «Греческий текст недоступен — показываем словарные формы»
- [x] При отсутствии alignment у стиха — fallback на word-layer
- [x] `buildWordEntries()` добавляет `strongNum: lexeme.strong` в каждую запись
...
- [x] Включить mode 4 и 5 в `top-bar.js` (enabled: true, убрать «скоро»)
- [x] Включить mode 4 и 5 в `settings.js`
- [x] Onboarding: вариант 3 → mode=3, вариант 4 → mode=4, все буквы known
- [x] В onboarding добавить **визуальные примеры** разницы между режимами:
...
- [x] В `renderWindowed()`: если mode === 5, рендерить греческий текст как основной
- [x] Каждый греческий токен → `<span class="gr grc-token">` с data-атрибутами (w, lemma, morph, strong)
- [x] **Touch targets:** CSS для `.grc-token` — padding: 4px 0, margin: -4px 0, min-height: 44px (зона касания без визуального раздвижения текста). Для коротких слов (ὁ, ἡ, τό) — псевдоэлемент `::after` расширяет зону горизонтально
- [x] Под каждым стихом — русский текст приглушённым курсивом (settings.show.ruHint toggle)
- [x] Тап по греческому токену → подробная карточка (лемма, грамматика, перевод)
- [x] Токены из словаря пользователя (strong совпадает) → класс `.known` (подсветка `--progress`)
- [x] Кнопка «глаз» в режиме 5 показывает чистый русский текст
- [x] `compose.js`: режим 5 возвращает null — reading.js сам строит DOM
...
- [x] Добавить ещё 60+ частотных слов НЗ (существительные, глаголы, прилагательные)
- [x] Для каждого: ruMatches (выверенные по реальному тексту Синодального), ruExclude, refs, freqNT
- [x] Приоритет: слова, повышающие покрытие alignment (сейчас 99.5%)
- [x] Перегенерировать alignment: `npm run build:data`
- [x] Обновить тесты
...
- [x] В настройках слова (showWordSettings): добавить радио «только лемма» / «все формы»
- [x] При `forms: 'lemma'` — form-layer использует `grToken.lemma`
- [x] При `forms: 'all'` — form-layer использует `grToken.w` (реальную форму)
- [x] Сохранять выбор в IndexedDB
...
- [x] Добавить свёрнутый блок «Дополнительно» с чекбоксами:
  - `show.diacritics` — показывать диакритику (ударения, придыхания)
  - `show.strongs` — показывать Strong numbers в карточке
- [x] При `show.diacritics = false` — леммы в тексте показываются без диакритики (через `stripDiacritics` в compose.js)
...
- [x] Добавить sentinel в конец каждой главы (элемент с `data-chapter-end="N"`)
- [x] IntersectionObserver на sentinel'ы: при попадании во вьюпорт → `chaptersRead.add(N)`
- [x] Сохранять в `progress.reading.books[bookId].chaptersRead`
- [x] В левой панели навигации: показывать галочки у прочитанных глав
- [x] В экране прогресса: показывать «прочитано N из M глав»
...
- [x] Экран «О приложении»: описание, версия, ссылка на GitHub
- [x] Атрибуция: SBLGNT EULA (Society of Biblical Literature), Clear-Bible Alignments (CC-BY-SA), bolls.life API
- [x] Шрифты: Gentium Plus (SIL OFL)
- [x] Маршрут `#/about`
- [x] `README.md`: описание, установка (`npm install && npm run build:data`), запуск (`npm run dev`), тесты (`npm test`), архитектура
...
- [x] На десктопе (≥900px) в режиме 2: span'ам `.gr[data-letter]` добавить `title` с именем буквы
- [x] В левой панели: список глав текущей книги с галочками прочитанных
- [x] В левой панели: кнопка «Продолжить чтение» (переход к последней позиции скролла)
- [x] Исправить warning двойного импорта dictionary.js — static imports вместо dynamic: **перед заменой динамического import на статический проверить граф зависимостей на циклическую зависимость** (reading.js → dictionary.js → reading.js). Если цикл есть — оставить динамический, но вынести в именованный реэкспорт для устранения warning
...
- [x] Скрипт анализирует покрытие alignment по книгам и главам
- [x] Выводит топ-50 отсутствующих лемм (слов, которые часто встречаются но не покрыты)
- [x] Выводит список книг с наихудшим покрытием
- [x] Результат: рекомендации по расширению core.json
...
- [x] Пройти все 5 режимов на десктопе (1280px) и мобильном (375px)
- [x] Пройти онбординг с чистой IndexedDB
- [x] Проверить все экраны в светлой и тёмной теме
- [x] Проверить оффлайн: `npm run build && npm run preview`, открыть книгу, выключить сеть
...
- [x] `npm test` зелёный; engine покрыт тестами
- [x] `npm run build` без ошибок и warnings
- [x] Детерминизм: перерендер не меняет картину замен
- [x] Производительность: ленивый DOM-рендер, книга Луки скроллится без лагов
- [x] Оффлайн: оболочка + прочитанные книги работают без сети
- [x] Доступность: aria-атрибуты, **touch targets ≥ 44px** (`.gr` span'ы через padding + margin), `:focus-visible`, контраст ≥ 4.5:1
- [x] Alignment: трёхуровневый (Manual ID → Strong → Regex), покрытие manual alignment'ом задокументировано
- [x] Все 5 режимов работают, включая деградацию при отсутствии данных
- [x] Страница «О приложении» с лицензиями (SBLGNT EULA, Clear-Bible CC-BY-SA, Gentium Plus SIL OFL)
- [x] README.md с инструкциями
- [x] Пунктуация: полный regex включая греческий `;` (U+037E)
- [x] Онбординг: визуальные примеры разницы лемма/форма
- [x] Циклические зависимости проверены, vite build без warnings

...
- [x] Заменить `const SKIP_ONBOARDING = true` на чтение
      `localStorage.getItem('dev_skip_onboarding') === '1'` в try/catch.
- [x] Если `localStorage` недоступен — skip = false (онбординг работает).
- [x] Остальную логику гварда (она уже есть в `app.js:68-93`) не менять.
- [x] `npm test` зелёный.
...
- [x] Тест: каждая запись `hasAlignment=true` имеет Strong, присутствующий в
      `core.json` (фиксация текущего ограничения; при будущем расширении
      alignment тест осознанно обновят).
- [x] Тест: записей `hasAlignment=true` больше 50 (защита от случайного
      обнуления включаемого словаря).
- [x] Таб «Доступные» в ряду фильтров (`getFilteredList`:
      `item.hasAlignment === true`), счётчик «Найдено: N» работает.
- [x] Тултип и предупреждение в карточке: «Нет проверенного русско-греческого
...
- [x] Если `loadFrequency()` вернул `null`/пустой массив — рендерить
      fallback-список личных слов из `dict` + `coreLexicon`.
- [x] Fallback поддерживает карточку и действия: статус, `showInText`,
      `intensity`, `forms` (через тот же `showWordCard`/`buildWordCard`).
- [x] Если словарь пользователя пуст: «Частотный список недоступен. Личный
      словарь пока пуст.»
- [x] Кнопку массового добавления слов не возвращать.
- [x] `npm test` зелёный.
...
- [x] Деградация: без alignment словарных замен нет, буквенный слой остаётся.
- [x] Источник архитектуры: DEVELOPMENT_1.md (разделы 3–4); DEVELOPMENT_2/3 —
      архив.
- [x] Упомянуть `frequency.json`: корпусный частотный каталог top-1000,
...
- [x] Отметить `[x]` все чекбоксы выполненных задач (фазы 1–5).
- [x] В заголовок статуса добавить строку: «Выполнено в `153ec7d..af672da`;
      стабилизация и доводка до релиза — DEVELOPMENT_5.md».
- [x] Содержание задач не редактировать (это уже история).
- [x] `npm run build` зелёный.
- [x] Коммит.
...
- [x] `npm run build:data` зелёный (инварианты не ослаблены).
- [x] `npm test` зелёный, data-тесты не skip'аются.
- [x] `npm run build` зелёный.
- [x] Онбординг: чистая IndexedDB без dev-флага → онбординг, после
      завершения — чтение с введёнными буквами.
- [x] Режим 1: пустой `progress.letters` не даёт замен.
- [x] Режимы 1–2: введённые буквы заменяются детерминированно, интенсивность
      влияет только на буквы.
- [x] Режим 3: core-слово из словаря заменяется леммой по alignment.
- [x] Режим 3: Ин 1:1 «было» НЕ заменяется на γίνομαι (контрольный кейс).
- [x] Режим 3: стих без alignment не делает словарных замен.
- [x] Режим 4: реальные формы; невыровненные слова остаются русскими;
      per-word интенсивность «редко» заметно снижает частоту замен.
- [x] Режим 5: греческий основной текст работает как раньше.
- [x] Переключение режимов 1 → 3 → 4 → 5 без reload догружает `grc` один раз.
- [x] Деградация offline: режимы 3–4 — тост, буквенный слой жив; режим 5 —
      русский текст без падений.
- [x] Инспектор desktop: чтение → словарь → чтение — карточки работают везде.
- [x] Словарь desktop 1280px: клик по строке → карточка в инспекторе.
- [x] Словарь mobile 375px: клик по строке → шторка; нет горизонтального
      скролла; чекбокс — touch target ≥ 44px.
- [x] Словарь: таб «Доступные» — 98 строк с активными чекбоксами; «Все» —
      1000; disabled-строки с честным объяснением.
- [x] Словарь: поиск латиницей `kai`, `theos`, `autos`, `logos` находит
      слова (новый translit).
- [x] Словарь: статус/чекбокс из карточки обновляют строку точечно, прокрутка
      не сбрасывается; карточка после «Добавить» показывает настройки.
- [x] Fallback без `frequency.json`: личный словарь виден, управляется.
- [x] Включение чекбокса слова в словаре → слово появляется в тексте режима 3
      (после возврата на чтение); выключение — исчезает.
- [x] Долгий тап (показ оригинала) работает на заменах.
- [x] Книга с минимальным выравниванием (Тит) читабельна в режиме 3.
- [x] Светлая и тёмная тема: чтение и словарь.
- [x] `git status`: только ожидаемые файлы; raw-источники
      (`docs/greek-nt-frequency-sources/raw/`) не закоммичены.
- [x] Коммит с отмеченным чеклистом.

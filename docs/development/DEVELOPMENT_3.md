...
| 5 | Данные (v1.0.1) | 2026-06-12: ручное выравнивание Clear-Bible оказалось внутренне рассогласованным в upstream (target-ID ссылаются на неопубликованную токенизацию RUSSYN; доказательство — Мф 1:1: υἱοῦ → запятая; ~54% словарных пар декодировались в чужие слова, что маскировалось рантайм-валидацией form-layer). Выравнивание перестроено детерминированно: Strong + ruMatches + сопоставление вхождений по порядку; verify() проверяет все пары корпусными инвариантами (границы, уникальность, 100% ruMatches-согласие, монотонность повторов) + точечные проверки Ин 1:1, Мк 1:1, 1Кор 1:1 (хиазм), Ин 3:16. Попутно: лицензии исправлены (SBLGNT — CC-BY 4.0, не EULA; CC-BY-SA у alignment был ошибкой), удалены неиспользуемые upstream-файлы (~17 МБ), убран хак «все буквы активны при пустом прогрессе», версия в «О приложении» берётся из package.json. | исправлено ✅ |
